...
serve(async (req) => {
    ...
    try {
        ...
        if (fetchError) {
            ...
            return new Response(JSON.stringify({ error: `Failed to fetch pending payments: ${fetchError.message}` }), {
                status: 500,
        ...
        for (const payment of pendingPayments) {
            ...
            try {
                console.log(`Checking Stripe PI status for ${payment.stripe_payment_intent_id} (DB ID: ${payment.id})`);
                ...
                if (paymentIntent.status === 'succeeded') {
                    ...
                    console.log(`Stripe PI ${paymentIntent.id} status is '${paymentIntent.status}'. Preparing DB update to 'failed'.`);
                    dbUpdateData = {
                        status: 'failed',
                    ...
                    console.log(`Stripe PI ${paymentIntent.id} status is '${paymentIntent.status}'. No DB update needed.`);
                }
                ...
                if (dbUpdateData) {
                    ...
                    if (updateError) {
                        console.error(`Failed to update payment ${payment.id} status in DB:`, updateError.message);
                        failedCount++;
                ...
                console.error(`Error retrieving or processing Stripe PI ${payment.stripe_payment_intent_id} for payment ${payment.id}:`, stripeError instanceof Error ? stripeError.message : stripeError);
                failedCount++;
                // Continue to next payment
            }
        } // End for loop

        const summary = `Sync completed. Checked: ${pendingPayments.length}, Updated: ${updatedCount}, Failed checks/updates: ${failedCount}.`;
        console.log(summary);
        ...
        console.error("Unhandled error in sync-pending-payments:", e.message);
