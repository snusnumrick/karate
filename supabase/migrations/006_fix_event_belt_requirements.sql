-- Fix check_event_registration_eligibility function to properly check belt requirements
-- Create a helper function to get belt rank order for comparison
...
-- Update the check_event_registration_eligibility function to include belt requirement checks
...
BEGIN
    ...
    IF event_record.min_belt_rank IS NOT NULL THEN
        ...
        IF student_belt_order < min_belt_order THEN
            RETURN jsonb_build_object(
                'eligible', false, 
                'reason', 'Student belt rank too low (requires at least ' || event_record.min_belt_rank || ' belt)'
            );
        END IF;
    END IF;
    ...
    IF event_record.max_belt_rank IS NOT NULL THEN
        ...
        IF student_belt_order > max_belt_order THEN
            RETURN jsonb_build_object(
                'eligible', false, 
                'reason', 'Student belt rank too high (maximum allowed is ' || event_record.max_belt_rank || ' belt)'
            );
        END IF;
    END IF;
