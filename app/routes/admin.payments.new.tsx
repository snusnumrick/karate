...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    throw new Response("Failed to load data for new payment form.", { status: 500, headers: Object.fromEntries(headers) });
  }
}
...
export async function action({ request }: ActionFunctionArgs): Promise<TypedResponse<ActionData>> {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    ...
    return json<ActionData>({ error: "Server configuration error." }, { status: 500, headers: Object.fromEntries(headers) });
  }
  ...
  try {
    ...
    return json<ActionData>({ error: "An unexpected error occurred while recording the payment." }, { status: 500, headers: Object.fromEntries(headers) });
  }
}
