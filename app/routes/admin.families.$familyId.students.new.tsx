...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (Object.keys(fieldErrors).length > 0) {
    ...
    return json({ error: "Please fill in all required fields.", fieldErrors, formData: Object.fromEntries(formData) }, { status: 400 });
  }
  ...
  try {
    ...
    // Check if it's a Supabase error for more details
     if (typeof error === 'object' && error !== null && 'code' in error) {
        ...
        console.error('[Admin Add Student Action] Supabase error code:', (error as any).code);
     }
    return json({
      error: errorMessage,
      fieldErrors, // Return field errors if they existed before the try block
      formData: Object.fromEntries(formData) // Return form data
    }, { status: 500 });
  }
}
...
export default function AdminAddStudentPage() {
  ...
  const getFormData = (key: string) => actionData?.formData?.[key] ?? '';
  const getFieldError = (key: string) => actionData?.fieldErrors?.[key];
      ...
      <Card>
          ...
          <CardContent>
              ...
              <Form method="post" className="space-y-6">
                  ...
                  <h3 className="text-lg font-semibold text-foreground mb-4 pb-2 border-b">Required Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* First Name */}
                      <div className="space-y-1">
                          <Label htmlFor="firstName">First Name<span className="text-destructive">*</span></Label>
                          <Input type="text" id="firstName" name="firstName" required defaultValue={getFormData('firstName')} aria-invalid={!!getFieldError('firstName')} aria-describedby="firstName-error"/>
                          {getFieldError('firstName') && <p id="firstName-error" className="text-sm text-destructive">{getFieldError('firstName')}</p>}
                      </div>
                      {/* Last Name */}
                      <div className="space-y-1">
                          <Label htmlFor="lastName">Last Name<span className="text-destructive">*</span></Label>
                          <Input type="text" id="lastName" name="lastName" required defaultValue={getFormData('lastName')} aria-invalid={!!getFieldError('lastName')} aria-describedby="lastName-error"/>
                           {getFieldError('lastName') && <p id="lastName-error" className="text-sm text-destructive">{getFieldError('lastName')}</p>}
                      </div>
                      {/* Birth Date */}
                      <div className="space-y-1">
                          <Label htmlFor="birthDate">Birth Date<span className="text-destructive">*</span></Label>
                          <Input type="date" id="birthDate" name="birthDate" required defaultValue={getFormData('birthDate')} aria-invalid={!!getFieldError('birthDate')} aria-describedby="birthDate-error" className="dark:[color-scheme:dark]"/>
                           {getFieldError('birthDate') && <p id="birthDate-error" className="text-sm text-destructive">{getFieldError('birthDate')}</p>}
                      </div>
                      {/* Gender */}
                      <div className="space-y-1">
                          <Label htmlFor="gender">Gender<span className="text-destructive">*</span></Label>
                          <Select name="gender" required defaultValue={getFormData('gender')}>
                              <SelectTrigger id="gender" aria-invalid={!!getFieldError('gender')} aria-describedby="gender-error">
                           ...
                           {getFieldError('gender') && <p id="gender-error" className="text-sm text-destructive">{getFieldError('gender')}</p>}
                      </div>
                      {/* T-Shirt Size */}
                      <div className="space-y-1">
                          <Label htmlFor="tShirtSize">T-Shirt Size<span className="text-destructive">*</span></Label>
                          <Select name="tShirtSize" required defaultValue={getFormData('tShirtSize')}>
                              <SelectTrigger id="tShirtSize" aria-invalid={!!getFieldError('tShirtSize')} aria-describedby="tShirtSize-error">
                           ...
                           {getFieldError('tShirtSize') && <p id="tShirtSize-error" className="text-sm text-destructive">{getFieldError('tShirtSize')}</p>}
                      </div>
                      {/* School */}
                      <div className="space-y-1">
                          <Label htmlFor="school">School<span className="text-destructive">*</span></Label>
                          <Input type="text" id="school" name="school" required defaultValue={getFormData('school')} aria-invalid={!!getFieldError('school')} aria-describedby="school-error"/>
                           {getFieldError('school') && <p id="school-error" className="text-sm text-destructive">{getFieldError('school')}</p>}
                      </div>
                      {/* Grade Level */}
                      <div className="space-y-1">
                          <Label htmlFor="gradeLevel">Grade Level<span className="text-destructive">*</span></Label>
                          <Select name="gradeLevel" required defaultValue={getFormData('gradeLevel')}>
                              <SelectTrigger id="gradeLevel" aria-invalid={!!getFieldError('gradeLevel')} aria-describedby="gradeLevel-error">
                           ...
                           {getFieldError('gradeLevel') && <p id="gradeLevel-error" className="text-sm text-destructive">{getFieldError('gradeLevel')}</p>}
                      </div>
                  </div>

                  <h3 className="text-lg font-semibold text-foreground mt-6 mb-4 pb-2 border-b">Optional Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Belt Rank */}
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                          ...
                          <Input type="tel" id="cellPhone" name="cellPhone" defaultValue={getFormData('cellPhone')} />
                      </div>
                      {/* Special Needs */}
                      <div className="space-y-1 md:col-span-2">
                          <Label htmlFor="specialNeeds">Special Needs (Leave blank if NONE)</Label>
                          <Input type="text" id="specialNeeds" name="specialNeeds" defaultValue={getFormData('specialNeeds')} />
                      </div>
                      {/* Allergies */}
                      <div className="space-y-1 md:col-span-2">
                          <Label htmlFor="allergies">Allergies (Leave blank if NONE)</Label>
                          <Textarea id="allergies" name="allergies" rows={3} defaultValue={getFormData('allergies')} />
                      </div>
                      {/* Medications */}
                      <div className="space-y-1 md:col-span-2">
                          <Label htmlFor="medications">Medications (Leave blank if NONE)</Label>
                          <Textarea id="medications" name="medications" rows={3} defaultValue={getFormData('medications')} />
                      </div>
                      ...
                      <div className="space-y-1 md:col-span-2">
                          ...
                          <Textarea id="immunizationNotes" name="immunizationNotes" rows={3} defaultValue={getFormData('immunizationNotes')} />
                      </div>
                  </div>
