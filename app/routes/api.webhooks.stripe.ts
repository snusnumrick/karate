...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const clientReferenceId = session.client_reference_id; // Your Supabase payment ID
    ...
    if (paymentStatus === 'paid') {
      ...
      if (session.payment_intent && typeof session.payment_intent === 'string') {
        try {
          ...
          console.error(`Error retrieving Payment Intent ${session.payment_intent}: ${piError.message}`);
    ...
    if (dbStatus === "succeeded") {
        try {
          ...
          console.error(`Failed to update payment status for session ${stripeSessionId}: ${updateError.message}`);
