...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (request.method !== "POST") {
    console.log("/logout action - Method not allowed:", request.method);
    // Set Allow header for 405 response
    headers.set("Allow", "POST");
    return new Response("Method Not Allowed", { status: 405, headers });
  }

  console.log("/logout action - Signing out user...");
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const { headers } = getSupabaseServerClient(request); // Get headers for response
  // Set Allow header for 405 response
  headers.set("Allow", "POST");
