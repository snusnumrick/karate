...
export async function loader() {
  ...
  const classes: ClassWithSchedule[] = (classesData || []).map(classItem => {
    ...
    if (!rawProgram || rawProgram.engagement_type !== 'program') {
      return null;
    }
  ...
  }).filter((classItem): classItem is ClassWithSchedule => Boolean(classItem));

  const scheduleSummary = {
    days: DEFAULT_SCHEDULE.days,
    time: DEFAULT_SCHEDULE.timeRange,
...
export default function CurriculumIndex() {
  ...
  const formatClassTime = (time: string) => {
    ...
    return new Date(`2000-01-01T${normalized}`).toLocaleTimeString("en-CA", {
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    });
                                ...
                                >
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-gray-900 dark:text-white font-medium">
                                      {formatClassTime(schedule.start_time)}
                                    </span>
                                  </div>
