...
export default function AttendanceReportPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Filter Form */}
      <Form method="get" className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg mb-6 flex flex-wrap items-end gap-4">
        <div>
          <Label htmlFor="startDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start Date</Label>
          <Input type="date" id="startDate" name="startDate" defaultValue={filterParams.startDate} className="w-full md:w-auto dark:bg-gray-700"/>
        </div>
        <div>
          <Label htmlFor="endDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Date</Label>
          <Input type="date" id="endDate" name="endDate" defaultValue={filterParams.endDate} className="w-full md:w-auto dark:bg-gray-700"/>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {reportData.overallPresent} Present / {reportData.overallAbsent} Absent ({reportData.overallTotal} Total Records)
            </p>
          </CardContent>
        </Card>
            ...
            <TableBody>
              {reportData.studentReports.map((student) => (
                <TableRow key={student.id}>
                  ...
                  <TableCell className="text-right font-semibold">{student.attendanceRate}%</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
