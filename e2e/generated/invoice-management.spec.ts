...
test.describe('Invoice Management', () => {
  ...
  test('smoke: /admin/invoice-entities/:id/ loads successfully', async ({ page }) => {
    // Collect JS errors before navigation
    const errors = [];
    page.on('pageerror', err => errors.push(err.message));

    await page.goto('/admin/invoice-entities/:id/');
  ...
  test('smoke: /admin/invoice-entities/:id/edit loads successfully', async ({ page }) => {
    // Collect JS errors before navigation
    const errors = [];
    page.on('pageerror', err => errors.push(err.message));

    await page.goto('/admin/invoice-entities/:id/edit');
