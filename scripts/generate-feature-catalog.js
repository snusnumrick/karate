...
/**
 * Auto-generate Feature Catalog from Codebase
 *
 * This script analyzes the codebase and generates a feature catalog
 * similar to standard_prd.json but automatically from code structure.
 *
...
const CONFIG = {
  ...
  featurePatterns: {
    ...
    'Student Management': {
      routePatterns: ['**/routes/**/*student*.{ts,tsx}'],
    ...
    'Waiver System': {
      ...
      componentPatterns: ['**/components/**/*Waiver*.{ts,tsx}']
    },
    'Event Management': {
...
function getTechStack() {
  ...
  if (deps['pg']) techStack.push('PostgreSQL client');
...
/**
 * Analyze feature based on files
 */
function analyzeFeature(featureName, files) {
  // Count file types
  ...
  let description = `Feature detected with ${files.length} file(s)`;
  ...
  if (utils.length > 0) parts.push(`${utils.length} util(s)`);
...
async function generateCatalog() {
  ...
  const catalog = {
    meta: {
      ...
      note: 'This is automatically generated. For manual overrides, edit standard_prd.json'
    },
  ...
  for (const [featureName, patterns] of Object.entries(CONFIG.featurePatterns)) {
    ...
    if (files.length > 0) {
      const feature = analyzeFeature(featureName, files);
