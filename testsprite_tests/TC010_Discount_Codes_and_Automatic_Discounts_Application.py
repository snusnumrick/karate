...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to access admin login page.
        ...
        # -> Check if there is a way to reset password or recover admin access, or try to register or find another way to access admin features.
        frame = context.pages[-1]
        ...
        # -> Input admin email into the reset password form and send reset link.
        frame = context.pages[-1]
        ...
        # -> Return to login page and attempt to test discount application from user perspective without admin setup.
        frame = context.pages[-1]
        ...
        # -> Click on 'register for classes' link to start enrollment process for testing discount application from user perspective.
        frame = context.pages[-1]
        # Click 'register for classes' link to start enrollment
        elem = frame.locator('xpath=html/body/div/main/div/div/div/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Fill registration form with valid data to create a new user account for enrollment.
        frame = context.pages[-1]
        # Input first name
        ...
        # -> Fill phone number and postal code fields, then submit the registration form.
        frame = context.pages[-1]
        # Input phone number
        ...
        # -> Click on 'Already a customer? Click here to login.' link to login as existing user and proceed with enrollment.
        frame = context.pages[-1]
        # Click 'Already a customer? Click here to login.' link to go to login page
        ...
        # -> Click 'Complete Profile Now' to update missing profile information.
        frame = context.pages[-1]
        # Click 'Complete Profile Now' link to complete user profile
        elem = frame.locator('xpath=html/body/div/main/div/div/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Fill Home Address, City, select Province, and click Save & Continue to complete profile.
        frame = context.pages[-1]
        # Input Home Address
        ...
        frame = context.pages[-1]
        # Click Province combobox to open options
        elem = frame.locator('xpath=html/body/div/main/div/div/div[3]/div[2]/form/div/div[2]/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select 'British Columbia' as the province and click 'Save & Continue' to complete profile.
        frame = context.pages[-1]
        # Select 'British Columbia' from province dropdown options
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click 'Save & Continue' button to submit profile completion and proceed to enrollment.
        frame = context.pages[-1]
        # Click 'Save & Continue' button to submit profile completion form
        elem = frame.locator('xpath=html/body/div/main/div/div/div[3]/div[2]/form/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        try:
            ...
            raise AssertionError("Test plan failed: Discount templates, codes, and automatic discounts did not apply properly during enrollment and checkout, including validation and expiry behaviors.")
